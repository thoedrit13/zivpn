#!/bin/bash
# Zivpn UDP Module installer + Dynamic Auto Exclude
# Creator Zahid Islam (Modified for Configurable Ports)
...
echo -e "====================================="
# ให้ผู้ใช้กำหนดพอร์ตตอนติดตั้ง
read -p "Enter Target Port (Press enter for Default '5666'): " input_target_port
TARGET_PORT=${input_target_port:-5666}
...
# สร้างไฟล์ Config สำหรับเก็บค่าให้แก้ภายหลังได้
cat <<EOF > /etc/zivpn/ports.conf
# ZIVPN Port Configuration
# สามารถแก้ไขค่านี้ได้ตามต้องการ และสั่ง 'systemctl restart zivpn' เพื่ออัปเดตระบบ
TARGET_PORT=$TARGET_PORT
EXCLUDE_PORTS="$EXCLUDE_PORTS"
EOF

# สร้าง Script สำหรับจัดการ iptables โดยดึงค่าจากไฟล์ Config
cat <<'EOF' > /usr/local/bin/zivpn-iptables.sh
#!/bin/bash
# โหลดค่าจากไฟล์ Config
...
# ค้นหาและลบกฎเดิมของ ZIVPN ออกทั้งหมด (โดยใช้ comment)
# ต้องลบจากบรรทัดล่างขึ้นบน (sort -nr) เพื่อไม่ให้บรรทัดขยับ
...
# สร้างไฟล์ Systemd ให้เรียกสคริปต์ iptables ก่อนเริ่มทำงาน (ExecStartPre)
